...
import * as championData from '@/assets/champion.json';
import * as itemData from '@/assets/item.json'

const uselessItems = [
   1101, 1102, 1103, 1104, 1035, 1039, 1040,
   ...
   2403, 2419, 2420, 2421, 2423, 2424,
   3330, 3340, 3363, 3364, 3400, 3513, 3599, 3600,
   3901, 3902, 3903,
   4403, 4638, 4643,
   7025, 7027, 7028, 7050,
]
const Ornnaments = [7000, 7001, 7002, 7003, 7004, 7005, 7006, 7007, 7008, 7009, 7010, 7011, 7012, 7013, 7014, 7015, 7016, 7017, 7018, 7019, 7020, 7021, 7022, 7023, 7024, 7026]
const armorPenItems = [
...
armorPenItems.forEach(item => {
   const isPercentage = item.percentage !== 0
   const newKey = isPercentage ? "PercentageArmorPenetrationMod" : "FlatArmorPenetrationMod"
   itemData.data[item.id].stats[newKey] = isPercentage ? item.percentage : item.lethality
})
magicPenItems.forEach(item => {
   const isPercentage = item.percentage !== 0
   const newKey = isPercentage ? "PercentageMagicPenetrationMod" : "FlatMagicPenetrationMod"
   itemData.data[item.id].stats[newKey] = isPercentage ? item.percentage : item.flat
})
