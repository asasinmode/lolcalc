...
export function calculateChampionStats(source: DamageSource): IStatsCalculationResult {
	...
	if (source.calculateStatsHooks.all.value.postItemTotal) {
		for (const hook of source.calculateStatsHooks.all.value.postItemTotal) {
			hook(source, { itemPassivesStats, itemTotalStats, championPassiveStats, totalMultipliersStats }, { calculatedVariables, debuffs, effectVars, miscDebug });
		}
	}
	...
	if (source.calculateStatsHooks.all.value.onRuneShards) {
		for (const hook of source.calculateStatsHooks.all.value.onRuneShards) {
			hook(source, { isRanged, runeShardStats, baseStats, adaptiveForceMeta }, { calculatedVariables, debuffs, effectVars, miscDebug });
		}
	}
