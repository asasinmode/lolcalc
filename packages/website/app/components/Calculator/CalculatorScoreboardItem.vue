...
function updateComputedStats(stats: IChampionStat[]) {
	for (const championStat of stats) {
		...
		for (let i = 0; i <= championStat.values.length - 1; i++) {
			...
			value.rawBonus = props.value.stats.value.bonus[value.stat];
			...
			if (championStat.iconTextureKey === 'armor' || championStat.iconTextureKey === 'magicResist' || baseValue) {
...
@layer components {
	...
	[data-scoreboard-item] {
		...
		.header {
			...
			grid-template-columns: repeat(5, max-content) minmax(0, 1fr) max-content;
			...
			@media (width < 1194px) and ((width >= 1079px) or (width < 600px)) {
				& {
					...
					grid-template-columns: repeat(3, max-content) minmax(0, 1fr) max-content;
				}
			}

			@media ((((width < 1680px) and (width >= 1079px)) or (width < 840px)) and ((height < 660px) and (height >= 280px))) or ((width < 460px) and ((height < 800px) and (height >= 280px))) {
				& {
		...
		.clear-remove,
		.manipulate {
			--at-apply: '-mbe-[calc(0.5px+0.5*var(--header-gap-y))] self-end';
		}

		.move-down,
		.duplicate,
		.expand-collapse {
			--at-apply: '-mbs-[calc(0.5px+0.5*var(--header-gap-y))] self-start z-1';
		}
		...
		.manipulate {
			--at-apply: 'size-[--manipulate-btn-size] hidden';
		...
		.select-champion {
			...
			.select-champion-level {
				...
				> label > span:last-child {
					--at-apply: 'size-[--select-level-size] bg-black text-white b b-[--ui-btn-border-clr] text-center text-[length:15px]/4 rounded-full grid-center';
		...
		.select-runes {
			...
			@media (width < 512px) {
				& {
					--at-apply: 'justify-self-end -translate-x-[--select-runes-items-translate]';
				}
			}
		...
		.select-items {
			--at-apply: 'self-center relative ms-[--select-items-runes-gap]';
			grid-area: select-items;

			@media (width < 512px) {
				& {
					--at-apply: '-translate-x-[--select-runes-items-translate]';
				}
			}
		...
		> :where(details) {
			...
			.stats {
				...
				@media (width < 460px) {
					& {
						--at-apply: 'grid-rows-2 justify-self-center';
					}
			...
			.effects {
				...
				@media (width < 460px) {
					& {
						--at-apply: 'justify-self-center block-auto pbs-1';
					}
				}
...
@layer overrides {
	...
	.mirrored [data-scoreboard-item][data-group='sources'] {
		...
		.select-runes {
			@media (width < 512px) {
				& {
					--at-apply: 'justify-self-start translate-x-[--select-runes-items-translate]';
				}
			}
		...
		.select-items {
			--at-apply: 'me-[--select-items-runes-gap] ms-0';
